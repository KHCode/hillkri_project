...
pols.post('/', check_accepts, post_pol, get_a_pol, (req, res) => {
    res.status(201).json(res.locals.pol);
});

pols.get('/:pol_id', check_accepts, get_a_pol, (req, res) => {
    res.status(200).json(res.locals.pol);
});

pols.get('/', check_accepts, get_pols, (req, res) => {
    res.status(200).json(res.locals.pols);
});

pols.put('/:pol_id', check_accepts, get_a_pol, edit_a_pol, (req, res) => {
    res.status(200).json(res.locals.edited_pol);
});

pols.patch('/:pol_id', check_accepts, get_a_pol, edit_a_pol, (req, res) => {
    res.status(200).json(res.locals.edited_pol);
});
...
pols.post('/:pol_id/actual_team/:member_id', check_accepts, get_a_pol, get_a_member, join_actual_team, edit_a_pol, edit_a_member, (req, res, next) => {
